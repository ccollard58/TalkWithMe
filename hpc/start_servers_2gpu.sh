...
# Start or restart the TalkWithMe services on an already allocated 2-GPU node
# (e.g. gpu-l40s). Run this from the node while the allocation is active; it
# does not submit a job. TTS and STT share GPU 0; the LLM gets GPU 1 to itself
# (see hpc/start_servers_h100sxm.sh for the 4-GPU variant with one GPU each).
#
...
echo "Starting llama-server on $NODE_HOST:$LLM_PORT (GPU $LLM_GPUS)"
(
